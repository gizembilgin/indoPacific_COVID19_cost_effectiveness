...
VE_waning_distribution_expander <- function(VE_waning_distribution, 
  ...
  workshop = data.frame()
  if (nrow(vaccination_history_LOCAL[vaccination_history_LOCAL$schedule == "booster",])>0){ #if booster dose exists
    for (this_dose in unique(vaccination_history_LOCAL$dose[vaccination_history_LOCAL$schedule == "booster"])){ # for each booster dose
      for (this_vax in unique(vaccination_history_LOCAL$vaccine_type[vaccination_history_LOCAL$schedule == "booster" & vaccination_history_LOCAL$dose == this_dose])){ # for each booster type
        
        # First Choice = exact primary dose + booster dose combination
        this_combo = local_VE_waning_distribution %>% 
          filter(schedule == "booster" & 
                   dose == this_dose & 
                   primary_if_booster %in% unique(vaccination_history_LOCAL$FROM_vaccine_type[vaccination_history_LOCAL$dose == this_dose & vaccination_history_LOCAL$vaccine_type == this_vax]) &
        ...
        if (nrow(this_combo) == 0){
          this_vax_mode = unique(vaccination_history_LOCAL$vaccine_mode[vaccination_history_LOCAL$vaccine_type == this_vax])
          this_combo = local_VE_waning_distribution %>% 
  ...
  for (loop_dose in 2:max(vaccination_history_LOCAL$dose)){
    ...
    if ("age_group" %in% colnames(issues_pt1)){
      issues = issues_pt1 %>% 
        left_join(issues_pt2,by = c("strain", "vaccine_type", "dose", "outcome", "days", "waning", "vaccine_mode", "schedule","age_group")) %>%
        group_by(schedule,vaccine_mode,strain,outcome,vaccine_type,days,waning,.add = TRUE) %>%
        filter((is.na(upper_dose) == FALSE & upper_dose<lower_dose))
    } else{
      issues = issues_pt1 %>% 
        left_join(issues_pt2,by = c("strain", "vaccine_type", "dose", "outcome", "days", "waning", "vaccine_mode", "schedule")) %>%
        group_by(schedule,vaccine_mode,strain,outcome,vaccine_type,days,waning,.add = TRUE) %>%
        filter((is.na(upper_dose) == FALSE & upper_dose<lower_dose))
    }
