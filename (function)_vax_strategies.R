...
vax_strategy <- function(vax_strategy_start_date,       # start of hypothetical vaccination program
  ...
  ##### SETUP ###################################################################
  ### WARNINGS 
  if (vax_strategy_start_date <= max(vaccination_history_TRUE$date) + 7){ 
    warning ('Your hypothetical vaccine campaign start date needs to be in the future! (one week buffer)')
  }
