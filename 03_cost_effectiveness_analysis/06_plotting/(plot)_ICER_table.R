# This script creates a table containing the expected mean values for the 1000 probabilistic model runs
...
cost_column <- crossing(
  cost_column,
  outcome = unique(Shiny_ICER_table$outcome[Shiny_ICER_table$outcome != "netCost"])
)
...
Shiny_ICER_table <- Shiny_ICER_table %>%
  ...
  ungroup() %>%
  select(perspective, discounting_rate, setting, booster_vax_scenario, antiviral_cost_scenario,
         antiviral_type, antiviral_target_group, outcome, count_outcome_averted, net_cost, ICER, LPI_ICER, UPI_ICER)
...
if (INPUT_ICER_table = "net"){
  ...
  table_S3_1 <- CommandDeck_result %>%
    ...
    ungroup()%>%
    select(-evaluation_level, -discounting_rate, -perspective, 
           -antiviral_cost_scenario, -antiviral_type, -LPI, -UPI, -sd, -variable_type) %>%
  ...
  table_S3_2 <- CommandDeck_result %>%
    ...
    filter(outcome %in% c("death","QALYs","interventionCost","healthcareCostAverted",
                          "productivityLoss","societal_netCost","healthcare_netCost","societal_ICER_QALYs","healthcare_ICER_QALYs") &
             (perspective == "societal perspective" | outcome %in% c("healthcare_netCost","healthcare_ICER_QALYs"))) %>%
    ungroup()%>%
    select(-evaluation_level, -discounting_rate, -perspective,-antiviral_cost_scenario, 
           -antiviral_type, -sd, -variable_type,-LPI,-UPI,-LPI_new,-UPI_new) %>%
    pivot_wider(names_from = outcome,values_from = mean) %>%
    mutate(scenario = case_when(
      booster_vax_scenario == "no booster" & antiviral_target_group == "no antiviral"                    ~ "Baseline with no booster or oral antiviral program",
      booster_vax_scenario == "no booster" & antiviral_target_group == "adults with comorbidities"       ~ "No booster, oral antiviral to high-risk adults",
    ...
    select(setting, scenario, death, QALYs, interventionCost, healthcareCostAverted,
           productivityLoss, healthcare_netCost, societal_netCost, healthcare_ICER_QALYs, societal_ICER_QALYs)
