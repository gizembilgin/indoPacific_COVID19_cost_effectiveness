# This function creates the 'worker' for each parallel core running the cost-effectiveness model
...
CEA_worker <- function(
  ...
  for (ticket in 1:numberOfRunsPerCluster) {
    ...
    MASTER_antiviral_simulations <- sample_transmission_model_runs(
      LIST_CEA_settings,
      LIST_booster_vax_scenarios,
      LIST_antiviral_elig_groups,
      LIST_antiviral_types,
    ...
    outcomesAvertedEstimation <- estimate_outcomes_averted(
    ...
    interventionCost_estimates <- estimate_intervention_costs(
      LIST_CEA_settings_mod,
      MASTER_antiviral_simulations,
      TORNADO_PLOT_OVERRIDE,
      LIST_antiviral_cost_scenario,
    ...
    healthcareCostEstimation <- estimate_healthcare_costs_averted(
      LIST_CEA_settings_mod,
      MASTER_antiviral_simulations,
      TORNADO_PLOT_OVERRIDE,
      toggle_uncertainty = TOGGLE_uncertainty
    ...
    ###(3/3) CEA per setting
    this_result <- summarise_this_CEA_run(
      DECISION_include_net,
