...
### (3) Reduced length of stay of patients who receive oral antivirals
### These distributions are later sampled from in stochastic runs of the CEA model
...
fit_lognormal <- function(mean,LB,UB){
  ...
  minimise_this_lognormal <- function(sd) {
    ...
    return((LB - LB_estimate)^2 +  (mean - mean_estimate)^2 + (UB - UB_estimate)^2)
  }
