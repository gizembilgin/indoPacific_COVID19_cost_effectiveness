...
for (d in D:1){
  if (d == D){
    ...
    if (date_now == date_start){ #run once per model run to save time
      ...
      workshop = workshop %>% left_join(suppressor, by = c("date", "vaccine_type", "dose", "age_group", "risk_group"))
...
if (nrow(vaccination_history_FINAL[vaccination_history_FINAL$dose == 8,])>0){
  ...
  if(booster_type %in% unique(vaccination_history_TRUE$vaccine_type)){
  ##ONLY if vaccine type for hypothetical is the same type as preexisting do we have to suppress pre existing; otherwise the shape of the distribution stays the same
  
  warning('vaccine occupany tracker is adding lots of time to the run')
