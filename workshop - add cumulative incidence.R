...
for (j in 1:length(settings_to_plot)){
  ...
  workshop_pt1$total_cases = workshop_pt1$total_cases[workshop_pt1$vax_scenario == "all willing adults vaccinated with a primary schedule"] - workshop_pt1$total_cases
  ...
  workshop_pt2$total_high_risk_cases = workshop_pt2$total_high_risk_cases[workshop_pt2$vax_scenario == "all willing adults vaccinated with a primary schedule"] - workshop_pt2$total_high_risk_cases
  
  workshop_pt3 = workshop_pt1 %>%
  ...
  workshop = workshop_pt3 %>% 
    left_join(workshop_pt4) %>%
    mutate(doses_per_case_averted = total_cases/overall,
           doses_per_high_risk_case_averted = total_high_risk_cases/overall) %>%
...
cases_averted_log %>%
  ...
  select(setting,vax_scenario_short,doses_per_case_averted,doses_per_high_risk_case_averted)
