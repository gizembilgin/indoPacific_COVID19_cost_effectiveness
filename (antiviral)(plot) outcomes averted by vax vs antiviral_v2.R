...
workshop = RECORD_antiviral_model_simulations %>% 
  ...
  filter(intervention %in% c("oral antiviral","booster dose")) %>%
...
workshop = RECORD_antiviral_model_simulations %>% 
  ...
  summarise(median = median(value), LQ = quantile(value,probs=0.25), UQ = quantile(value,probs=0.75),
            ...
            intervention_doses_delivered = median(intervention_doses_delivered,na.rm=TRUE)) %>%
...
to_print = workshop %>% 
  ungroup()%>%
  filter(outcome %in% LIST_outcomes) %>%
  select(setting_beta,outcome,vax_scenario_short,antiviral_target_group,intervention_doses_delivered,median,LQ,UQ) %>%
  mutate(output = paste(round(median)," (",round(LQ),"-",round(UQ),")",sep="")) %>%
  select(-median,-LQ,-UQ)
to_print$vax_scenario_short = factor(to_print$vax_scenario_short, levels = c("catch-up campaign all adults",
                                                                             "booster to all",
...
### SM molnupiravir
LIST_outcomes = list('hosp', 'severe_disease','YLL','death')
this_result = "percentage"
this_result = "doses_per_outcome_averted"
...
for (a in 1:length(LIST_outcomes)) {
  ...
  plot_list[[a]] = ggplot(data = workshop[workshop$outcome == this_outcome,]) +
    geom_pointrange(aes(x=median,y=vax_scenario_short,
                        xmin=LQ,xmax=UQ,
                        color=as.factor(antiviral_type)))  +
    labs(title = paste(this_outcome),
         color = '',
         shape = "") +
    ylab('')+
...
workshop = RECORD_antiviral_model_simulations %>%
  ...
  mutate(intervention = case_when(
    intervention == "antiviral 2023-01-01" ~ "overall impact",
    intervention == "antiviral after booster 2023-01-01" ~ "after booster rollout",
    intervention == "antiviral prior to booster 2023-01-01" ~ "before booster rollout"
  )) %>%
  group_by(setting_beta,intervention,outcome,antiviral_type,antiviral_target_group,evaluation_group,
           vax_scenario,vax_scenario_risk_group,result,vax_scenario_short) %>%
  summarise(median = median(value), LQ = quantile(value,probs=0.25), UQ = quantile(value,probs=0.75),
...
to_print = workshop %>% 
  ungroup()%>%
  filter(outcome %in% LIST_outcomes) %>%
  select(setting_beta,outcome,vax_scenario_short,intervention,antiviral_target_group,median,LQ,UQ) %>%
  mutate(output = paste(round(median)," (",round(LQ),"-",round(UQ),")",sep="")) %>%
  select(-median,-LQ,-UQ)
to_print$vax_scenario_short = factor(to_print$vax_scenario_short, levels = c("catch-up campaign all adults",
                                                                             "booster to all",
