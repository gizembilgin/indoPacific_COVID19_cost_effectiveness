...
### This script creates figures S3.1 and S3.2 for the Supplementary Material

CommandDeck_CONTROLS = list(
                                LIST_CEA_settings = list("PNG_low_beta","TLS","FJI","IDN"),
                                LIST_perspectives = c("societal"),
                                LIST_antiviral_cost_scenario = c(
                                  #"low_generic_cost"
                                  "middle_income_cost" 
                                  #"high_income_cost"
                                  ),
...
### Figure S3.1
figure_S3_1 <- list()
figure_S3_1 [[1]] = ggplot(outcomesAvertedEstimation$QALY_breakdown) + geom_col(aes(x=outcome_source,y=count_outcomes)) +
...
figure_S3_1 [[2]] =ggplot(interventionCost_estimates) + geom_col(aes(x=gsub(" 2023-01-01","",gsub(" 2023-03-01","",intervention)),y=cost)) +
  facet_grid(setting ~., scales = "free") + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  xlab("")+
...
figure_S3_1 [[4]] =ggplot(healthcareCostEstimation$healthcareCosts_breakdown) + geom_col(aes(x=patient_type,y=cost)) +
  facet_grid(setting ~., scales = "free") + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  xlab("")+
...
#collecting costs by components in a unified dataframe
...
net_cost_by_component$cost_category <- factor(net_cost_by_component$cost_category,
...
net_cost_by_component = net_cost_by_component%>% arrange(cost_category)
...
for (this_setting in unique(net_cost_by_component$setting)){
  figure_S3_2[[length(figure_S3_2) + 1]] <- ggplot(net_cost_by_component[net_cost_by_component$setting == this_setting,]) + 
    geom_col(aes(y=cost_category,x=cost,fill=as.factor(cost_category))) +
...
ggarrange(plotlist = figure_S3_2, ncol = 1, nrow = 4,  legend = "none",align = "hv")
