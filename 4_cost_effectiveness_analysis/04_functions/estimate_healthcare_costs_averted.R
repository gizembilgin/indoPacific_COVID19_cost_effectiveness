# This function estimates healthcare costs averted
...
estimate_healthcare_costs_averted <- function(
  ...
  TRANSLATED_antiviral_simulations <- MASTER_antiviral_simulations %>%
    #don't care about age-specific incidence since we don't have age-specific estimates for costs of accessing healthcare
  ...
  healthcare_access <- data.frame(
    setting = c("FJI", "IDN", "PNG", "TLS"),
  ...
  if (toggle_uncertainty == "rand"){
    ...
    for (row_num in 1:nrow(cost_estimates)){
      if (cost_estimates$patient_type[row_num] == "inpatient"){
        ...
        if (cost_estimates$count_outcomes[row_num]<0) {multiplier = -1} else {multiplier = 1}
        
        if (cost_estimates$count_outcomes[row_num] < cutoff_sampling){ #if sampling less than a million times
          this_sample <- data.frame(est = rnorm(abs(cost_estimates$count_outcomes[row_num]), 
                                                mean = cost_estimates$param1[row_num], 
                                                sd = cost_estimates$param2[row_num])) %>%
            mutate(est = case_when(
              est <0 ~ 0,
              TRUE ~ est
            ))
        ...
        if (cost_estimates$count_outcomes[row_num] < cutoff_sampling){ #if sampling less than a million times
          this_sample <- data.frame(est = rlnorm(cost_estimates$count_outcomes[row_num], 
                                                 meanlog = cost_estimates$param1[row_num], 
                                                 sdlog = cost_estimates$param2[row_num])) %>%
